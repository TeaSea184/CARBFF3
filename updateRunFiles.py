...
def updateRunFiles(molecule_name, phi, psi, email, path):
    print(phi)
    print(psi)
    print("Executing updateRunFiles")
#read in all files  
    output = f"output/{molecule_name}"
    ...
    for line in fileinput.input(f"{output}/runPMF.sh", inplace=True):
        print(line)
        if line.strip().startswith("#SBATCH--mail-user="):
        ...
        if line.strip().startswith("cd"):
            ...
            print(line,end='')
        if line.strip().startswith("/opt/exp_soft/NAMD_2.13_Linux-x86_64-multicore-CUDA/namd2run.conf>"):
...
if __name__ == "__main__":
    ...
    phi = output[0]
    psi = output[1]
    updateRunFiles(name,phi,psi, email, path)
#Example usage
